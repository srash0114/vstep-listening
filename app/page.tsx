  ...
  return (
    <div className="w-full flex flex-col" style={{ background: "var(--bg-base)" }}>
      <style>{`
        ...
        .glass-panel { background: color-mix(in srgb, var(--bg-surface) 75%, transparent); backdrop-filter: blur(12px); border: 1px solid var(--border-default); box-shadow: 0 20px 40px rgba(0,0,0,0.1); }
      ...
      <section className="scroll-mt-16 snap-start lg:snap-always w-full flex flex-col relative text-center lg:text-left z-10 lg:pt-0 lg:pb-0 overflow-hidden overflow-y-visible lg:overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-center items-center w-full px-4 sm:px-8 py-10">
          ...
          >
            ...
            <motion.div variants={zoomIn} className="relative w-full aspect-square max-w-[380px] sm:max-w-[400px] lg:max-w-none lg:w-full lg:h-[400px] xl:h-[500px] mx-auto order-1 lg:order-2 perspective-1000">
              ...
              {/* Floating Badges */}
              <motion.div animate={{ y: [0, -15, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} className="absolute -top-8 sm:-top-8 -right-2 sm:-right-10 p-3 sm:p-5 rounded-3xl z-20 flex items-center gap-3" style={{ background: "var(--bg-glass)", border: "1px solid var(--border-glass)", backdropFilter: "blur(12px)" }}>
                <div className="text-2xl sm:text-3xl">🎯</div>
                <div>
                  <p className="font-black text-sm sm:text-base tracking-tight" style={{ color: "var(--text-on-glass)" }}>VSTEP 8.5+</p>
                  <p className="text-xs font-semibold uppercase tracking-wider mt-0.5" style={{ color: "var(--text-on-glass-muted)" }}>Mục tiêu đỗ</p>
                </div>
              </motion.div>
              <motion.div animate={{ y: [0, 15, 0] }} transition={{ repeat: Infinity, duration: 5, ease: "easeInOut", delay: 1 }} className="absolute -bottom-6 sm:-bottom-10 -left-2 sm:-left-10 p-3 sm:p-5 rounded-3xl z-20 flex items-center gap-3" style={{ background: "var(--bg-glass)", border: "1px solid var(--border-glass)", backdropFilter: "blur(12px)" }}>
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-emerald-500/20 flex items-center justify-center">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                </div>
                <div>
                  <p className="font-black text-sm sm:text-base tracking-tight" style={{ color: "var(--text-on-glass)" }}>Chấm điểm ngay</p>
                  <p className="text-xs font-semibold uppercase tracking-wider mt-0.5" style={{ color: "var(--text-on-glass-muted)" }}>AI tự động feedback</p>
                </div>
              </motion.div>
            </motion.div>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, delay: 0.4 }} className="max-w-7xl mx-auto w-full mt-auto py-5 px-5 rounded-full hidden sm:block" style={{ background: "var(--bg-glass)", border: "1px solid var(--border-glass)", backdropFilter: "blur(12px)" }}>
          <div className="px-4 flex justify-between gap-4 overflow-x-clip">
            {[
              ...
              <div key={i} className="flex items-center gap-3 text-sm font-semibold" style={{ color: "var(--text-muted)" }}>
                <span className="text-violet-500"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{f.icon}</svg></span>
                <span>{f.text}</span>
              </div>
            ))}
          </div>
      ...
      <section id="exams" className="scroll-mt-16 snap-start snap-always w-full flex flex-col justify-center px-4 py-0 sm:py-10">
        ...
        >
          ...
          {!loading && !error && exams.length === 0 && (
            <motion.div variants={zoomIn} className="text-center py-32 rounded-[2.5rem]" style={{ background: "var(--bg-glass)", border: "1px dashed var(--border-default)" }}>
              <div className="text-6xl mb-6 opacity-80">🎧</div>
              <p className="text-xl font-black mb-2" style={{ color: "var(--text-primary)" }}>{t("Kho đề đang được cập nhật", "Library is updating")}</p>
              <p style={{ color: "var(--text-muted)" }}>{t("Quay lại sau nhé!", "Check back later!")}</p>
            </motion.div>
          )}
          ...
          {!loading && exams.length > 0 && (
            ...
            >
              ...
              <button
                onClick={scrollLeft}
                className="hidden sm:flex absolute left-2 lg:-left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full items-center justify-center opacity-0 group-hover/slider:opacity-100 transition-all hover:scale-110"
                style={{ background: "var(--bg-glass)", border: "1px solid var(--border-glass)", backdropFilter: "blur(12px)", color: "var(--text-on-glass)", boxShadow: "0 0 20px rgba(0,0,0,0.2)" }}
              ...
              <button
                onClick={scrollRight}
                className="hidden sm:flex absolute right-2 lg:-right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full items-center justify-center opacity-0 group-hover/slider:opacity-100 transition-all hover:scale-110"
                style={{ background: "var(--bg-glass)", border: "1px solid var(--border-glass)", backdropFilter: "blur(12px)", color: "var(--text-on-glass)", boxShadow: "0 0 20px rgba(0,0,0,0.2)" }}
          ...
          >
            ...
            <motion.div variants={slideRight} className="relative w-full order-2 lg:order-1 flex justify-center py-2 lg:py-8 mt-4 lg:mt-0">
              ...
              <motion.div
                ...
                className="absolute -top-3 -right-0 sm:top-auto sm:-bottom-8 sm:-right-4 lg:-right-8 p-3 sm:p-5 rounded-[1rem] sm:rounded-2xl shadow-2xl z-20 w-[85%] max-w-[200px] sm:max-w-none sm:w-64 backdrop-blur-md"
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4 mb-2 sm:mb-3">
                  <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-full bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30 shrink-0">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  </div>
                  <div>
                    <p className="text-[11px] sm:text-sm font-bold tracking-wide" style={{ color: "var(--text-on-glass)" }}>Listening Part 1</p>
                    <p className="text-emerald-500 text-[10px] sm:text-xs font-bold mt-0.5">8/8 Correct</p>
                  </div>
                </div>
                <div className="w-full rounded-full h-1 sm:h-1.5 mt-1.5 sm:mt-2 overflow-hidden" style={{ background: "var(--border-glass)" }}>
                  <motion.div initial={{ width: 0 }} whileInView={{ width: "100%" }} transition={{ duration: 1, delay: 0.5 }} className="bg-emerald-500 h-full rounded-full" />
                </div>
              </motion.div>
              <div className="relative w-full max-w-[280px] sm:max-w-[400px] lg:max-w-[500px]">
                <div className="relative w-full aspect-[4/3] rounded-[1.5rem] lg:rounded-[2.5rem] overflow-hidden group flex flex-col" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-default)", boxShadow: "0 10px 30px rgba(0,0,0,0.15)" }}>
                  {/* Mac OS window header */}
                  <div className="h-8 sm:h-10 w-full flex items-center px-4 sm:px-5 gap-1.5 sm:gap-2 shrink-0" style={{ background: "var(--bg-overlay)", borderBottom: "1px solid var(--border-subtle)" }}>
                  ...
                  {/* Image content */}
                  <div className="relative flex-1 w-full" style={{ background: "var(--bg-base)" }}>
            ...
            <div className="order-1 lg:order-2 flex flex-col items-center lg:items-start text-center lg:text-left">
              <motion.div variants={slideLeft} className="mb-6 w-full">
                ...
                <p className="text-sm sm:text-base max-w-lg mx-auto lg:mx-0 leading-relaxed" style={{ color: "var(--text-muted)" }}>
              ...
              <div className="relative flex flex-col gap-3 sm:gap-4 w-full max-w-lg mx-auto lg:mx-0">
                {[
                  ...
                  <motion.div variants={slideUp} key={s.step} className="relative flex gap-3 sm:gap-4 text-left group items-start">
                    ...
                    <div className="p-3 sm:p-4 rounded-2xl flex-1 transition-all duration-300 shadow-sm group-hover:shadow-md" style={{ background: "var(--bg-glass-hover)", border: "1px solid var(--border-glass)" }}>
          ...
          >
            <motion.div variants={zoomIn} className="rounded-[3rem] px-8 sm:px-16 py-16 relative overflow-hidden glass-panel"
              ...
              <div className="relative z-10 flex flex-col items-center text-center max-w-2xl mx-auto">
                ...
                <h3 className="text-3xl sm:text-5xl font-black mb-6 tracking-tight drop-shadow-md" style={{ color: "var(--text-on-glass)" }}>
                ...
                <p className="text-base sm:text-lg mb-10" style={{ color: "var(--text-on-glass-muted)" }}>
                  {t(
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto justify-center">
                  ...
                  <Link href="/login" className="px-10 py-4 rounded-2xl text-base font-bold transition-all duration-300" style={{ border: "1px solid var(--border-default)", color: "var(--text-on-glass)", background: "var(--bg-glass)" }}>
      ...
      {isScrolldown && (
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={scrollToTop}
            className="relative flex items-center justify-center w-12 h-12 lg:w-14 lg:h-14 rounded-full focus:outline-none overflow-hidden"
            style={{ border: "1px solid var(--border-default)", boxShadow: "0 8px 30px rgba(0,0,0,0.2)" }}
            aria-label="Scroll to top"
          >
            {/* Default background */}
            <div className="absolute inset-0 backdrop-blur-xl transition-opacity duration-500 group-hover:opacity-0" style={{ background: "var(--bg-glass)" }} />
            ...
            {/* Inner highlight */}
            <div className="absolute inset-0 rounded-full transition-colors duration-300" style={{ border: "1.5px solid var(--border-glass)" }} />
            ...
            <div className="relative z-10 flex flex-col items-center justify-center transition-transform duration-300 group-hover:-translate-y-1">
              <svg className="w-5 h-5 lg:w-6 lg:h-6 drop-shadow-md" style={{ color: "var(--text-on-glass)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function ExamCard({ exam }: { exam: Exam }) {
  ...
  return (
    <Link href={`/exam/${exam.id}`} className="group block h-full w-full">
      <div
        className="h-full min-h-[440px] rounded-[2rem] overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl flex flex-col relative"
        style={{
      ...
      >
        {/* Hover glow */}
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-[2rem]" style={{ background: `radial-gradient(circle at 50% 0%, ${lc.border}, transparent 70%)` }} />

        {/* Thumbnail */}
        <div className="w-full h-44 shrink-0 relative overflow-hidden" style={{ borderBottom: "1px solid var(--border-glass)" }}>
          <div
            className="absolute inset-0 bg-cover bg-center"
            style={{ backgroundImage: `url('https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=600&auto=format&fit=crop')`, opacity: 0.8 }}
          />
          ...
          <div className="absolute bottom-4 left-6">
            {exam.level && (
              <span
                className="inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-black uppercase tracking-wider px-3 py-1.5 bg-black/40 backdrop-blur-md rounded-xl shadow-sm"
                style={{ color: lc.text, border: `1px solid ${lc.border}` }}
              >
                {exam.level}
              </span>
            )}
        ...
        <div className="p-6 flex flex-col flex-1 z-10">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div
              className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg transition-transform duration-500 group-hover:scale-[1.15] group-hover:rotate-6"
          ...
          <h3
            className="font-extrabold text-xl leading-snug mb-3 transition-all group-hover:text-transparent bg-clip-text"
          ...
          {exam.description && (
            <p className="text-sm line-clamp-2 mb-6 flex-1 transition-colors" style={{ color: "var(--text-on-glass-muted)" }}>
              {exam.description}
            </p>
          )}
          ...
          <div className="mt-auto">
            <div className="flex flex-wrap gap-2 mb-6">
              {[
                ...
                <span key={s.label} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl font-bold" style={{ background: "var(--bg-overlay)", border: "1px solid var(--border-glass)", color: "var(--text-on-glass-muted)" }}>
                  <span className="opacity-70">{s.icon}</span> {s.label}
                </span>
              ))}
            </div>
            <div className="w-full py-4 rounded-xl text-sm font-black text-center text-white transition-all duration-300 shadow-[0_5px_15px_rgba(0,0,0,0.15)] opacity-90 group-hover:opacity-100" style={{ background: `linear-gradient(${lc.gradient})` }}>
